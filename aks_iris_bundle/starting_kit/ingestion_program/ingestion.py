...
DATA_PATH = "/mnt/inputdata"
OUTPUT_PATH = "/mnt/output"
PRIVATE_OUTPUT_PATH = "/mnt/private"

logging.basicConfig(
   level=logging.DEBUG,
   format='%(asctime)s:%(levelname)s:%(name)s:%(message)s',
   filename = join(OUTPUT_PATH, 'run1.log'),
   filemode='a'
)
# PF BB
...
if __name__=="__main__" and debug_mode<4:
    ...
    if len(argv)==1: # Use the default input and output directories if no arguments are provided
        ...
        input_dir = os.path.abspath(argv[1]) # input
        output_dir = os.path.abspath(argv[2]) # output
        program_dir = os.path.abspath(argv[3]) # ing prog
        submission_dir = os.path.abspath(argv[4]) # model prog\sub prog
    if verbose:
