...
def read_array(filename, blob=False):
    ''' Read array and convert to 2d np arrays '''
    if blob == False: # default
        array = np.genfromtxt(filename, dtype=np.float)
        if len(array.shape) == 1:
            array = array.reshape(-1, 1)
        return array
    else:
        list_array = [i.split(' ')[0:3] for i in np.array(filename.split('\n')) if len(i.split(' ')[0:3]) > 1]
        int_list_array = [map(int, i) for i in list_array]
        array = np.array(int_list_array, dtype=np.float)
        return array
