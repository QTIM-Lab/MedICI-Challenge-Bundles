...
# local dev and proper, will fix later
IN='/mnt/in'
OUT='/mnt/out'
REFERENCE='/mnt/solution'

using_blob = True

if using_blob:
    IN=argv[1] # is to become prediction blob, "container name"
...
# Running locally
    ...
    prediction_files = os.listdir(IN)
    if len(prediction_files) != 1:
        raise Exception('Only one csv file allowed.')
    else:
        with open(os.path.join(IN, prediction_files[0]), 'r') as pred:
            pred_images, pred_classes = get_predictions(pred)
